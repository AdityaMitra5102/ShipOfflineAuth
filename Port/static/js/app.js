...
async function completeAuthentication() {
    ...
    // Start with back camera index 2
    await scanner.start(scanner.availableCameras[2]?.deviceId || null);
}
...
async function handleScan(scan) {
    ...
    try {
        ...
        if (response.status === 200 && payload && payload.status === 'success') {
            showResult('✓ Authentication successful for '+payload.ship, 'ok');
            setStatus('Done.', 'ok');
        } else {
...
cancelScan.addEventListener('click', async () => {
    if (confirm('Switch to other back camera?')) {
        // Try the other back camera (index 3)
        await scanner.start(scanner.availableCameras[3]?.deviceId || null);
    } else {
        stopScanner();
