...
async start(deviceId = null) {
    ...
    try {
        if (!deviceId && this.availableCameras.length === 0) {
            await this.getCameras();
        }
        
        // Display available cameras on the page
        const cameraInfo = this.availableCameras.map((cam, i) => 
            `${i}: ${cam.label || '(unlabeled)'}`
        ).join('\n');
        
                
        if (!deviceId && this.availableCameras.length > 0) {
            deviceId = this.availableCameras[0].deviceId;
        }
